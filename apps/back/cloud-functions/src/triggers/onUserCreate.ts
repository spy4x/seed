...
import { UserGetQuery, LogSegment, LogService, UserDetailsDto } from '@seed/back/api/shared';
...
async function handler(logSegment: LogSegment, user: UserRecord): Promise<void> {
  ...
  const userInDB = await queryBus.execute<UserGetQuery, null | UserDetailsDto>(new UserGetQuery(user.uid));
